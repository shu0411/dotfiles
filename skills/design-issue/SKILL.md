...
Issue駆動で開発を進めるリポジトリにおける「設計フェーズ」専用のSkill。
人間が最初に書く簡単な要件（要求のみ）を入力として、既存コードを調査しながら
実装可能な粒度の仕様書に育て、GitHub Issue本文そのものを更新する。

プロジェクト固有のルール（`AGENTS.md` / `CLAUDE.md` など）があれば、その記述を優先する。
...
- リポジトリ直下の `AGENTS.md` / `CLAUDE.md`（アーキテクチャ、データモデル、
  APIエンドポイント、拡張時の注意点）
- 要求に関連するディレクトリ・モジュール（フロントエンド、バックエンド、
  インフラなど、リポジトリの構成に応じたもの）
- 似た既存機能があれば、その実装パターン・命名規則
- 設計・仕様関連のドキュメント（`docs/` 配下など、リポジトリにあれば）
